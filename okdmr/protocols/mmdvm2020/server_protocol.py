...
class Mmdvm2020ServerProtocol(protocols.DatagramProtocol):
    ...
    def get_peer(
        ...
        return self.create_peer(address=address, repeater_id=repeater_id) if create_if_not_exists else None
    ...
    def create_peer(self, address: Tuple[str, int], repeater_id: int):
        ...
        self.peers[repeater_id] = MMDVM2020Peer(address=address, repeater_id=repeater_id)
    ...
    def get_repeater_id(
        ...
        elif hasattr(packet.command_data, "data") and hasattr(packet.command_data.data, "repeater_id"):
            return packet.command_data.data.repeater_id
        prettyprint(packet)
        raise Exception("packet no repeater_id")

    @staticmethod
    def parse_mmdvm2020_data(
            packet: bytes
    ) -> Optional[Tuple[Mmdvm2020, int]]:
        try:
    ...
    def datagram_received(
        ...
        if response:
            ...
            if check:
                ...
                print(f"{dgram_id} Response sent {check[0].command_data.__class__.__name__}")
            else:
                print(f"{dgram_id} Response is not valid Mmdvm2020 packet {response.hex()}")
        else:
